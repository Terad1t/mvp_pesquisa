...
def test_parser_entra_no_contrato_principal_para_senado():
    resultado = _extrair_hibrido(PDF_REAL, "SENADOR")
    bloco = resultado.cargos[0]

    assert bloco.cargo == "SENADOR"
    assert len(bloco.candidatos) == 8
    assert bloco.candidatos[0].nome == "ALEXANDRE CURI"
    assert bloco.candidatos[0].votos == 507
    assert bloco.ns_nr == Decimal("27.7")
    assert bloco.brancos_nulos == Decimal("7.0")
...
def test_modo_completo_nao_chama_gemini_quando_todas_tabelas_passam(monkeypatch):
    monkeypatch.setattr("main.extrair", lambda caminho: (_ for _ in ()).throw(AssertionError("Gemini não deveria ser chamado")))

    resultado = _extrair_hibrido(PDF_REAL, None)

    assert {bloco.cargo for bloco in resultado.cargos} == {"GOVERNADOR", "PRESIDENTE", "SENADOR"}
...
def test_posicao_parcialmente_ausente_bloqueia():
    bloco = CargoExtraido.model_validate(
        {
            "cargo": "GOVERNADOR",
            "candidatos": [
                {"posicao": 1, "nome": "A", "partido": "AA", "porcentual": 10, "porcentagem_valida": 10},
                {"nome": "B", "partido": "BB", "porcentual": 10, "porcentagem_valida": 10},
            ],
            "ns_nr": 1,
            "brancos_nulos": 1,
        }
    ...
    assert any("Posição extraída para alguns candidatos" in o.mensagem for o in ocorrencias)
