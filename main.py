...
_CARGOS_PARSER = {"GOVERNADOR", "PRESIDENTE"}
...
def _extrair_hibrido(caminho_pdf: Path, cargo_filtro: str | None) -> ResultadoExtracao:
    ...
    if alvo in _CARGOS_PARSER:
        ...
        # Senado ainda exige Gemini, mas os cargos que o parser resolver devem
        # prevalecer sobre os blocos equivalentes devolvidos pela IA.
...
def _processar_cargo(
    ...
    Devolve (status, resultado, ocorrências), onde status é um destes:
        ...
        "pulado"    -> cargo sem regra de cálculo implementada (ex.: Senado);
                        resultado é None, mas os dados brutos continuam em
                        `bloco` para quem chamou decidir o que fazer com eles
    """
...
def processar(resultado: ResultadoExtracao, top_n: int = 3, cargo_filtro: str | None = None) -> int:
    ...
    pendentes: set[str] = set()  # extraído, mas sem regra de cálculo ainda (ex.: Senado)
    ...
    if finais:
        _salvar_json_combinado(resultado, finais, faltando=faltando, pendentes=pendentes)
...
def _salvar_json_combinado(
    ...
    e o JSON precisa distinguir os dois para quem for consumi-lo depois:
        ...
        cargos_pendentes -> foi extraído e validado estruturalmente, só não
                             tem fórmula de cálculo ainda (hoje, Senado). É
                             esperado, não é um erro — por isso NÃO derruba
                             "completo" para false.
...
def _processar_pdf(
    ...
    codigo = processar(resultado, top_n=top_n, cargo_filtro=cargo_filtro)
    # A saída padrão individual permanece pesquisa.json. O lote fornece um
    # nome baseado no PDF para evitar colisões entre relatórios do mesmo estado.
    if codigo == 0 and nome_saida != "pesquisa.json":
        origem = PASTA_SAIDAS / resultado.estado.replace(" ", "_") / "pesquisa.json"
        nome_saida = _nome_saida_disponivel(origem.parent, nome_saida)
        destino = origem.with_name(nome_saida)
        if origem.exists() and origem != destino:
